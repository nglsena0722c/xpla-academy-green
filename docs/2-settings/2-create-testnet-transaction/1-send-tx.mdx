...
import Ex02 from "@site/src/components/Sandpack/Ex02";
import Ex01_react from "@site/src/components/Sandpack/Ex01_react";
...
XPLA 블록체인에 트랜잭션을 생성하려면 XPLA 토큰으로 수수료를 지불해야 합니다. [이전 단계](/docs/settings/create-wallet/get-testnet-xpla)에서 테스트넷 XPLA 토큰을 받았으니, 수수료 걱정 없이 테스트넷에서 트랜잭션을 생성해봅시다. 

<Details summary="트랜잭션을 생성하려면 왜 수수료를 지불해야 하나요?">

블록체인에 트랜잭션은 어떻게 생성되는 걸까요? 블록체인의 시작부터 현재까지 기록된 모든 데이터를 저장하고 있는 사람들이 있습니다. 그들을 검증자(Validator)라고 부르죠. 검증자들은 새로 기록될 데이터를 네트워크에서 받아 검증하고, 문제가 없으면 블록체인에 추가하는 역할을 맡고 있습니다.

검증자들의 활동에는 많은 컴퓨팅 자원이 필요합니다. 우리는 컴퓨팅 자원을 사용하지 않는 대신에, 수수료를 지불하여 검증자들에게 데이터를 블록체인에 추가해달라고 요청하게 되는 것이죠. 이를 PoS(Proof of Stake) 방식이라고 합니다.

검증자에 관해 더 자세히 알고 싶다면 [Docs](https://docs.xpla.io/docs/full-node/manage-a-validator/validator-faq/)를 참고해보세요.
...
[이전 단계](/docs/settings/create-wallet/get-testnet-xpla)에서 Faucet 지갑에게 XPLA 토큰을 전송받았습니다. 이번엔 우리 지갑에서 Faucet 지갑으로 XPLA 토큰을 전송하는 트랜잭션을 만들어보겠습니다. 전송하는 금액은 25aXPLA (0.000000000000000025XPLA)입니다.
...
해시값이란 `다양한 길이의 데이터를 일정한 길이의 데이터로 변환한 값`입니다. 어떻게 변환하는지는 적용하는 알고리즘에 따라 달라집니다.
...
해시값의 특징으로는, 입력값이 조금만 달라져도 해시값이 바뀐다는 점, 해시값을 통해 입력값이 무엇인지는 알아내기 어렵다는 점 등이 있습니다.
...
XPLA 토큰이 소모되었다는 것을 통해, 우리는 트랜잭션이 잘 생성되었다고 간접적으로 알 수 있습니다.

<BrowserWindow url="https://xpla.io/" bodyStyle={{padding: "0px 0px 16px 0px"}}>
<div className="flex justify-end">
    <VaultImgCard imgSrc="/xpla-academy-green/img/2-settings/2-create-testnet-transaction/send-tx/sendtx-checkfee.png" />
</div>
</BrowserWindow>
